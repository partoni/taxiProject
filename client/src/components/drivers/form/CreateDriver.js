...
const CreateDriver=()=>{
    ...
    // const subscription = watch((value, { name, type }) => console.log(`VALUE______`,value, `NAME------`,name, `TYPE------`,type));
...
<div className={styles.form__item}>
        ...
        <label className={styles.form__field_file}>
            <input type='file' multiple className={styles.form__input_file} {...register('file')} />
            ...
            <div className={styles.img}>{imgList?imgList.map((item,id)=><img key={id} src={item}/>):''}</div>
        </label>
        </div>
